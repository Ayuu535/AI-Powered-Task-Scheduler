...
def sort_tasks_by_edf(tasks):
    """
    Enhanced Earliest Deadline First (EDF) algorithm implementation.
    
    Sorts tasks based on their deadline. Tasks with earlier deadlines come first.
    For tasks with the same deadline, tasks with higher priority values come first.
    ...
    # For tasks with the same deadline, sort by priority (higher priority first)
    sorted_incomplete = sorted(incomplete_tasks, key=lambda task: (task.deadline, -task.priority))
    
    # Sort completed tasks by deadline (earlier deadlines first)
    # For completed tasks, priority is less important but still considered
    sorted_completed = sorted(completed_tasks, key=lambda task: (task.deadline, -task.priority))
